                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'nowrap', gap: '10px' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', minWidth: 0 }}>
                      <span className="font-semibold text-sm text-primary truncate" style={{ maxWidth: '140px' }}>{source.name}</span>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '2px' }}>
                        <span className={`badge ${source.status === 'active' ? 'badge-success' : 'badge-muted'}`} style={{ fontSize: '9px', padding: '1px 6px' }}>
                          {source.status === 'active' ? 'Enabled' : 'Disabled'}
                        </span>
                        <span className="text-xs text-muted" style={{ fontSize: '10px' }}>Created: {format(new Date(source.createdAt), 'dd MMM yyyy')}</span>
                      </div>
                    </div>
